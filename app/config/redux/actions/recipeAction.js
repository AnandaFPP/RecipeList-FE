...
export const getAllRecipe = () => async (dispatch) => {
    try {
        const recipes = await axios.get(`http://172.20.10.2:8008/recipes/`);
...
export const getDetailRecipe = (recipes_id) => async (dispatch) => {
    try {
        const recipes = await axios.get(`http://172.20.10.2:8008/recipes/${recipes_id}`);
        const result = recipes.data.data;
...
export const getRecipeByUserId = (users_id) => async (dispatch) => {
    try {
        const recipes = await axios.get(`http://172.20.10.2:8008/recipes/users/${users_id}`);
        const result = recipes.data.data;
        dispatch({ type: "GET_RECIPE_BY_ID", payload: result });
    } catch (err) {
        console.error(err.message);
    }
};
        ...
        async (dispatch) => {
            try {
                ...
                const recipes = await axios.put(
                    `http://172.20.10.2:8008/recipes/${recipes_id}`,
                    formData,
                    {
                        headers: {
                            "Content-Type": "multipart/form-data",
